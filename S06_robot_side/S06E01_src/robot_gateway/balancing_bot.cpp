...
std::map<std::string, float> BalancingBot::get_observation() {
...
std::map<std::string, float> BalancingBot::policy(
    std::map<std::string, float> observation, 
    std::map<std::string, float> command
  ) {
  ...
  float target_pitch = lpf_pitch_cmd(
    pid_vel((M0 * observation["motor0_velocity"] + M1 * observation["motor1_velocity"]) / 2 - cmd_throttle)
  );
  float voltage_control = pid_stb(Offset_parameters - observation["pitch_angle"] + target_pitch);
