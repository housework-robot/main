...
class BalancingBot {
  ...
  public:
    ...
    std::map<std::string, float> get_observation();
    std::map<std::string, float> policy(std::map<std::string, float> observation, std::map<std::string, float> command);
    void step(std::map<std::string, float> action);
}; 
